...
public class CellProperties {
    ...
    public void createPdf(String dest) throws IOException {
        ...
        table.addCell(new Cell().add(new Paragraph("Rotated")).setRotationAngle(Math.PI / 18).setFont(font).setFontSize(8).setFontColor(Color.RED));
        ...
        Cell cell = new Cell().add(new Paragraph("Test")).setFixedPosition(100, 400, 350).setBorder(border).setBackgroundColor(Color.BLUE).setHeight(100).setHorizontalAlignment(HorizontalAlignment.CENTER);
        document.add(cell);
        document.close();
    }
}
